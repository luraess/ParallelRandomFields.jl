...
@views function RandomField3Dg()
    ...
    if do_viz
        ENV["GKSwstype"]="nul"; 
        if (me==0)
            if isdir("viz3D_gauss")==false mkdir("viz3D_gauss") end; loadpath = "./viz3D_gauss/"; anim = Animation(loadpath,String[])
    ...
    if do_save  file = matopen("Rnd3Dg.mat", "w"); write(file, "Rnd3D", Array(Yf_v)); close(file)  end
